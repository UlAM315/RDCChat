...
<head>
    ...
    <script>
        ...
        function addNewFriends(friendId){
            //用户提示
            var nickname = prompt("给对方一个爱的昵称吧~");
            if (nickname!=null && nickname!=''){
                //ajax请求
                $.post("MyFriends/addNewFriends", {friendId:friendId,nickname:nickname}, function (data){
                    if (data.flag){
                        alert("添加成功！快去聊天吧！");
                        location.href="${pageContext.request.contextPath}/NewFriends.jsp";
                    }else {
                        alert("添加失败！");
                    }
                });
            }else {
                alert("给对方一个爱的昵称吧~内容不能为空哦！");
            }
...
<div class="container">
    ...
    <table border="1" class="table table-bordered table-hover">
        ...
        <c:forEach items="${newFriends}" var="friend">
            <tr><
                <td><img src="${friend.avatarImg}"></td>
                <td>${friend.userName}</td>
                <td>${friend.idNumber}</td>
                <td>${friend.content}</td>
                <td>${friend.gender}</td>
                <td>
                    <a class="btn btn-default btn-sm" href="javascript:addNewFriends(${friend.userId});">确认添加</a>
                </td>
            </tr>
        </c:forEach>
