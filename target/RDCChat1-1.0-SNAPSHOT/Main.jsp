...
<head>
    ...
    <script type="text/javascript">
        /*function setReceiver(receiver) {
            /!*$("#receiver").val(obj.text);
            document.getElementById("reply-to").innerHTML = "接收人：" + obj.innerHTML;*!/
            location.href = "${pageContext.request.contextPath}/MyFriends/singleChat?receiver="+receiver;
        }*/
    </script>
    ...
    <script>
        ...
        function getGroupMessage(groupId){
            location.href="${pageContext.request.contextPath}/MyFriends/groupMessage?groupId="+groupId;
        }
                    ...
                    for (let i = 0; i < info.data.length; i++) {
                        if (info.data[i].nickname != null){
                            var friendsLists = '<li class="aFriend">'+
                                                ...
                                                '<span class="friendName"><a href="${pageContext.request.contextPath}/MyFriends/singleChat?receiver='+info.data[i].userName+'">'+info.data[i].nickname+'</a></span>'+
                                                '</li>';
                            friends_lis += friendsLists;
                        } else {
                            var friendsLists = '<li class="aFriend">'+
                                                '<a href="javascript:getFriendMessage('+info.data[i].friendId+');"><img src="/RDCChat1/'+info.data[i].avatarImg+'"></a>'+
                                                '<span class="friendName"><a href="${pageContext.request.contextPath}/MyFriends/singleChat?receiver='+info.data[i].userName+'">'+info.data[i].userName+'</a></span>'+
                                                '</li>';
                            friends_lis += friendsLists;
                        }
                    }
                    ...
                    for (let i = 0; i < info.data.length; i++) {
                        if (info.data[i].myGroupName != null){
                            var chatsLists = '<li class="aChat">'+
                                '<a href="javascript:getGroupMessage('+info.data[i].groupId+');"><img src="/RDCChat1/'+info.data[i].avatarImg+'"></a>'+
                                '<span class="chatName"><a href="${pageContext.request.contextPath}/MyFriends/groupChat?groupId='+info.data[i].groupId+'&groupName='+info.data[i].myGroupName+'">'+info.data[i].myGroupName+'</a></span>'+
                                '</li>';
                            $(".chatsLists").html(chatsLists);
                        } else {
                            var chatsLists = '<li class="aChat">'+
                                '<a href="javascript:getGroupMessage('+info.data[i].groupId+');"><img src="/RDCChat1/'+info.data[i].avatarImg+'"></a>'+
                                '<span class="chatName"><a href="${pageContext.request.contextPath}/MyFriends/groupChat?groupId='+info.data[i].groupId+'&groupName='+info.data[i].groupName+'">'+info.data[i].groupName+'</a></span>'+
                                '</li>';
                            $(".chatsLists").html(chatsLists);
                        }
                    }
...
<div class="chatBox" style="display: block;">
    ...
    <div class="chatRight">
        ...
        <div class="inputChatContent">
            <input id="receiver" hidden="hidden" value="" type="text">
...
<%--<script type="text/javascript">
    ...
    if ('WebSocket' in window) {
        websocket = new WebSocket("ws://localhost:8080/RDCChat1/websocket?userName="+userName+"&receiver="+receiver);
    }
    else {
